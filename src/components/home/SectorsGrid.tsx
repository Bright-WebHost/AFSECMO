...
// Cards are now sourced from the localized `sectors.items` array

// ─── Individual card ────────────────────────────────────────────────────────
function Card({ item }: { item: { id: string; prefix?: string; title?: string; subtitle?: string; desc?: string; tag?: string; image?: string } }) {
  ...
  return (
    ...
    >
      ...
      <img
        src={item.image}
        alt={item.title ? `${item.title} ${item.subtitle || ''}` : 'Sector image'}
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-350">
          ...
          <div>
            ...
            <p className="max-w-155 text-2xl font-light leading-[1.3] tracking-tight text-[#111] sm:text-3xl lg:text-[38px] lg:leading-[1.2]">
              {t(
                ...
                "We support mining, energy, infrastructure and logistics operations with practical coordination, reliable sourcing and field execution."
              )}
            </p>
          </div>
        ...
        >
          {(() => {
            const items = t('sectors.items', { returnObjects: true }) as any[];
